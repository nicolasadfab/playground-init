...
;var pgutil = {not_null:function(a) {
  ...
  var a = PG.Util.readCookie("unique-id"), b;
  if(!PG.Util.not_null(a)) {
    b = "abcdefghijklmnopkrstuvwxyz*-()#\x26" + (new Date).getTime();
    var c = "", d;
    for(d = b.length - 1;0 < d;d--) {
      c += b.charAt(Math.floor(Math.random() * (d + 1))), c += b.charAt(d)
    }
    b = c.slice(0, 20);
    PG.Util.createCookie("unique-id", b)
  }
  return PG.User.uid = a
